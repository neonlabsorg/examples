...
SPLToken constant _splToken = SPLToken(
  0xFf00000000000000000000000000000000000004
);
Metaplex constant _metaplex = Metaplex(
...
contract ERC20ForSpl {
  ...
  event ApprovalSolana(
    address indexed owner,
    bytes32 indexed spender,
    uint64 amount
  );
  ...
  constructor(bytes32 _tokenMint) {
    require(
      _splToken.getMint(_tokenMint).isInitialized,
      'ERC20: invalid token mint'
    );
    require(
      _metaplex.isInitialized(_tokenMint),
      'ERC20: missing MetaPlex metadata'
    );
  ...
  function claimTo(
    ...
    (bool status, ) = address(_splToken).delegatecall(
      abi.encodeWithSignature(
        'transfer(bytes32,bytes32,uint64)',
        from,
        toSolana,
        amount
      )
  ...
  function _burn(address from, uint256 amount) internal {
    ...
    require(
      ...
      'ERC20: burn amount exceeds uint64 max'
    );

    bytes32 fromSolana = _solanaAccount(from);

    require(
      _splToken.getAccount(fromSolana).amount >= amount,
  ...
  function _transfer(
    ...
    require(
      amount <= type(uint64).max,
      'ERC20: transfer amount exceeds uint64 max'
    );
    require(
      _splToken.getAccount(fromSolana).amount >= amount,
...
contract ERC20ForSplMintable is ERC20ForSpl {
  ...
  function mint(address to, uint256 amount) public {
    ...
    require(
      amount <= type(uint64).max,
      'ERC20: mint amount exceeds uint64 max'
    );
    require(
      totalSupply() + amount <= type(uint64).max,
      'ERC20: total mint amount exceeds uint64 max'
    );
