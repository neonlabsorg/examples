...
contract ERC721ForMetaplex is IERC165, IERC721, IERC721Metadata {
    SPLToken constant _splToken =
        SPLToken(0xFf00000000000000000000000000000000000004);
    Metaplex constant _metaplex =
        Metaplex(0xff00000000000000000000000000000000000005);
    ...
    function claim(bytes32 from, uint64 amount) external returns (bool) {
        ...
        require(
            account.state == SPLToken.AccountState.Initialized,
            "ERC721: invalid spl token account state"
        );
    ...
    function transferSolanaFrom(
        ...
        bytes32 fromSolana = _splToken.findAccount(
            keccak256(abi.encode(tokenId, from))
        );
